...
### `pqc encrypt` / `pqc decrypt` — крипто-схема алгебры архетипа (v0.3.6)

Реализация уравнения из алгебры смыслов (файл 285):
`p_{t+1} = a ⊗_ε p_t ⊕ m` → `p* = a ⊗_ε p* ⊕ m`,
восстановление `m = p* ⊕ (a ⊗_ε p*)`.

Ключ — готовый `precess --out` (контейнер v3 с гироскопом J): из
русел J детерминированно берутся плотные моды Im(P), проектор
`a = Σ (u uᵀ + v vᵀ)` **идемпотентен точно** (орто-невязка ~1e-16 —
RQ11), поэтому коллапс к `p*` — один такт. Сообщение — паттерн
`δ = (I−a)(ε·b)` на «холодных» компонентах (энергия мод ≤ 0.06/K),
ключевой поток — `a·p₀` (проекция фаз ключа). Блочный режим CBC c
IV; контейнер `.pqc` (magic `PQC1`, 64 B заголовок + фазы f32);

    pqc encrypt msg.bin --key archetype.pqw --out msg.pqc --seed 42
    pqc decrypt msg.pqc --key archetype.pqw --out plain.bin

Измерено на чекпоинте RQ11 (d_pol=4096, 2651 русло): K=8 мод,
ёмкость 3905 бит/блок, помеха проекции 0.0037 (порог 0.05), запас
декодирования 0.028, расширение ×43, 0.03 с на 3 КиБ; чужой ключ —
структурный отказ либо BER ≈ 50%. Честные границы: схема линейна
над скрытым подпространством (known-plaintext с числом блоков
≳ 2K восстанавливает S⊥; внутриблочная лавина минимальна) —
исследовательская демонстрация уравнения, не production-шифр.
...
- **v0.3.6 (здесь)** — ✅ RQ12: крипто-схема алгебры архетипа (файл
